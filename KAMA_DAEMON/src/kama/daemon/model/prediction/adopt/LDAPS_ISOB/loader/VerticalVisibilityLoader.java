...
public class VerticalVisibilityLoader
{
    ...
    private int JL = 16, JN = 24, MCT = 100, NMCT = 7, NR = 4, NT = 37;
    ...
    private String CITY_INFO_FILE = String.format("%s/%s", DaemonSettings.getCurrentWorkingDirectory(), "res/ldps_meteocity.dat");
    ...
    private double[][] stn = new double[NT][MCT];
    private double[][] stx = new double[NT][MCT];
    ...
    public double[] getWindSpeedListByTime(int stnid)
    {
        ...
        for (int i = 0; i < NT; i++)
        {
            temp[i] = Math.sqrt(Math.pow(suw[i][index], 2.) + Math.pow(svw[i][index], 2.));
        }
    ...
    private void _parseDataInternal2DArray(FloatBuffer fbuf, int n)
    {
        ...
        for (int m = 0; m < cities.size(); m++) // 10
        {
            srl[n][m] = fbuf.get();
        }
        ...
        for (int m = 0; m < cities.size(); m++) // 11
        {
            src[n][m] = fbuf.get();
        }
