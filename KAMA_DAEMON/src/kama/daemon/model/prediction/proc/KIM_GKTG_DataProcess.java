 ...
 */
public class KIM_GKTG_DataProcess extends DataProcessor
{
    private static final String DATAFILE_PREFIX = "kim_gktg";
    ...
    public KIM_GKTG_DataProcess(DaemonSettings settings)
    {
    ...
    protected void processDataByFileGroup(DatabaseManager dbManager, File[] dataFiles, ProcessorInfo processorInfo) throws Exception
    {
        ...
        List<File> gktgFileList = new ArrayList<File>();
        
        for(int i=0 ; i<dataFiles.length ; i++) {
        	
        	if(dataFiles[i].length() == 2276820349L) {
        		gktgFileList.add(dataFiles[i]);
        	}
        }
        
        dataFiles = gktgFileList.toArray(new File[gktgFileList.size()]);
