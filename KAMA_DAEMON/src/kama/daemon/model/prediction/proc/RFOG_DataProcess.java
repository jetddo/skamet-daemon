 ...
 */
public class RFOG_DataProcess extends DataProcessor
{
    private static final String DATAFILE_PREFIX = "rfog";
    private static final String FILE_TYPE_LFOG_VIS = "rfog_vis_[0-9]{12}.asc"; // lfog_visb.2016110512_000
    private static final String FILE_TYPE_LFOG_OUT_BIN = "rfog_out.[0-9]{4}.[0-9]{10}.bin"; // lfog_out.0090.2017011900.bin
    ...
    public RFOG_DataProcess(DaemonSettings settings)
    {
        super(settings, DATAFILE_PREFIX);
    }

    /**
     * 각 (하나의) 파일에 대한 데이터 처리 m함수
    ...
    private void processDataInternal_fogInfo(DatabaseManager dbManager, File file, ProcessorInfo processorInfo) throws Exception
    {
        ...
        //Log.print("INFO : fogInfo : " + fogInfoList.size());

        for (FogInfoLoader.FogInfo info : fogInfoList)
        {
            String queryTemplate = "MERGE INTO %%WORKSPACE%%.NMDL_RFOG d USING (SELECT ''{0}'' STN_ID, TO_DATE(''{1}'', ''YYYY-MM-DD HH24:mi:ss'') PREDCT_TIME, TO_DATE(''{2}'', ''YYYY-MM-DD HH24:mi:ss'') MDL_PROD_TIME, ''{3}'' FLOOR FROM dual) s ON (d.STN_ID = s.STN_ID AND d.PREDCT_TIME = s.PREDCT_TIME AND d.MDL_PROD_TIME = s.MDL_PROD_TIME AND d.FLOOR = s.FLOOR) WHEN MATCHED THEN UPDATE SET d.RFOG_VSBL = ''{4}'' WHEN NOT MATCHED THEN INSERT(STN_ID, PREDCT_TIME, MDL_PROD_TIME, FLOOR, RFOG_VSBL) VALUES (''{0}'', TO_DATE(''{1}'', ''YYYY-MM-DD HH24:mi:ss''), TO_DATE(''{2}'', ''YYYY-MM-DD HH24:mi:ss''), ''{3}'', ''{4}'')";
    ...
    {
        defineQueryFormat(INSERT_QUERY, "INSERT INTO %%WORKSPACE%%.NMDL_RFOG(STN_ID, PREDCT_TIME, MDL_PROD_TIME, FLOOR, RFOG_VSBL, TEMP, DEW_PT, HUMID, PRES) VALUES (''{0}'', TO_DATE(''{1}'', ''YYYY-MM-DD HH24:mi:ss''), TO_DATE(''{2}'', ''YYYY-MM-DD HH24:mi:ss''), ''{3}'', ''{4}'', ''{5}'', ''{6}'', ''{7}'', ''{8}'')");
        defineQueryFormat(TRUNCATE_QUERY, "DELETE FROM %%WORKSPACE%%.NMDL_RFOG");
        defineQueryFormat(DELETE_QUERY, "DELETE FROM %%WORKSPACE%%.NMDL_RFOG WHERE STN_ID = ''{0}'' AND PREDCT_TIME = TO_DATE(''{1}'', ''YYYY-MM-DD HH24:mi:ss'') AND MDL_PROD_TIME = TO_DATE(''{2}'', ''YYYY-MM-DD HH24:mi:ss'') AND FLOOR = ''{3}''");
    }
    //</editor-fold>
