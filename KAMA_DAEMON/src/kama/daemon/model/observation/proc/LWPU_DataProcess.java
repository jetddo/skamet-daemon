 ...
 */
public class LWPU_DataProcess extends WPU_DataProcess_ABS
{
    private static final String DATAFILE_PREFIX = "lwpu";
    private static final int DB_COLUMN_COUNT = 9;
    private static final int FILE_DATE_INDEX_POS = 2; // LWPU_47095_201609010000.DAT
    private static final int[] DB_PRIMARY_KEY_INDEXES = { 0, 1, 2 }; // TM, WMO_ID, HT
    private static final int[] DB_PRIMARY_KEY_INDEXES_OLD = { 0 }; // WMO_ID
    ...
    public LWPU_DataProcess(DaemonSettings settings)
    {
    ...
    {
        defineQueryFormat(INSERT_QUERY, "INSERT INTO %%WORKSPACE%%.LWPU(TM, WMO_ID, HT, LAT, LON, TD, WSPD, DATA_STT, IDX_SEQ) VALUES (TO_DATE(''{0}'', ''YYYY-MM-DD HH24:mi:ss''), ''{1}'', ''{2}'', ''{3}'', ''{4}'', ''{5}'', ''{6}'', ''{7}'', ''{8}'')");
        defineQueryFormat(DELETE_QUERY, "DELETE FROM %%WORKSPACE%%.LWPU WHERE TM = TO_DATE(''{0}'', ''YYYY-MM-DD HH24:mi:ss'') AND WMO_ID = ''{1}'' AND HT = ''{2}''");

        // ******* 기존 AAMI 버전 3D 파싱용 *******
        defineQueryFormat(INSERT_QUERY_OLD_1, "INSERT INTO %%WORKSPACE_OLD_AAMI%%.LWPU(WMO_ID, LAT, LON, H_HT, WIND, DT) VALUES  (''{0}'', ''{1}'', ''{2}'', ''{3}'', ''{4}'', TO_DATE(''{5}'', ''YYYYMMDDHH24mi''))");
        defineQueryFormat(INSERT_QUERY_OLD_2, "INSERT INTO %%WORKSPACE_OLD_AAMI%%.LWPU_B(WMO_ID, LAT, LON, H_HT, WIND, DT) VALUES  (''{0}'', ''{1}'', ''{2}'', ''{3}'', ''{4}'', TO_DATE(''{5}'', ''YYYYMMDDHH24mi''))");
        defineQueryFormat(DELETE_QUERY_OLD, "DELETE %%WORKSPACE_OLD_AAMI%%.LWPU WHERE WMO_ID=''{0}''");
    }
    //</editor-fold>
