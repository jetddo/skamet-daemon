...
import kama.daemon.model.observation.adopt.RDR.proc.RadarKmaWdConveter;
import kama.daemon.model.observation.adopt.RDR.proc.RadarKmaWdConveter2;

/**
 * @author chlee
 * Created on 2017-01-03.
 */
public class KMA_WD_DataProcess extends DataProcessor
{
    private static final String DATAFILE_PREFIX = "kma_wd";
    private static final int DB_COLUMN_COUNT = 3;
    private static final int FILE_DATE_INDEX_POS = 3; // RDR_CFQCZ_3D80_201609010000.bin.gz
    ...
    public KMA_WD_DataProcess(DaemonSettings settings)
    {
    ...
    protected void processDataInternal(DatabaseManager dbManager, File file, ProcessorInfo processorInfo) throws Exception
    {
        ...
        if(file.getName().endsWith("nc")) {
        	...
        	RadarKmaWdConveter2 converter = new RadarKmaWdConveter2(dbManager.Settings, processorInfo);        
            converter.parseRadarFile(file);
            
        } else {
        	
        	RadarKmaWdConveter converter = new RadarKmaWdConveter(dbManager.Settings, processorInfo);        
            converter.parseRadarFile(file);
        }
