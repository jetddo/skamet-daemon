...
public class KimAcimSectorTableGenerator {
	...
	private final String insertFileProcInfo = 
			...
			" (TO_DATE('{fileDt}', 'YYYYMMDDHH24MI'), '{fileName}', '{filePath}', SYSDATE, 'KIM_ACIM_CNVT') "; 
	...
	private Map<String, List<NetcdfDataset>> getAcimModelFileMap() {
		...
		// KIM ACIM 데이터가 datastore 에 이미 저장되어있다고 가정하고 수행
...
//	    	System.out.println("예측 시간 크기: " + fcstHourSize);
	...
	private void createLegend(Graphics2D g, int width, int height, int tableRightMargin, int tableBottomMargin, Font font) {
		...
		g.drawString("기반모델: KIM GDAPS", legendLeftMargin - legendTitle.length()*font.getSize()/3*2, legendTopMargin + font.getSize()*3); // 범례 텍스트 위치 조정);
	}
	...
	public static void main(String[] args) {
		...
		KimAcimSectorTableGenerator acim = new KimAcimSectorTableGenerator();
