...
public class KimGdpsNe36ImageGeneratorTest {
	...
	public KimGdpsNe36ImageGeneratorTest() {
	...
	public BoundLonLat generateImage(NetcdfDataset ncFile, String fileName, String savePath) {
		...
		try {
			...
			for(int j=0 ; j<1 ; j++) {
				...
				System.out.println("\t\t-> Start Read Variable");
				
				Variable var = ncFile.findVariable("Relative_humidity_isobaric");
				...
				float[][] values = GridCalcUtil.convertStorageToPrimitiveValuesReverse(var.read(rangeList).getStorage(), rows, cols);
				
				System.out.println("\t\t-> End Read Variable");
				...
				for(int k=0 ; k<rows ; k++) {
					...
					for(int l=0 ; l<cols ; l++) {
						...
						Color c = kimGdpsLegendFilter.getColor_Relative_humidity_isobaric(v);
	...
	public static void main(String[] args) {
		...
		KimGdpsNe36ImageGeneratorTest kimGktgImageGeneratorTest = new KimGdpsNe36ImageGeneratorTest();
                  
		try {
						
			NetcdfDataset ncFile = NetcdfDataset.acquireDataset("F:/data/datastore/KIM_GDPS/2026/05/10/00/kim_g128_ne36_pres_h000.2026051000.gb2", null);
			String fileName = "kim_g128_ne36_pres_h000.2026051000.gb2";
