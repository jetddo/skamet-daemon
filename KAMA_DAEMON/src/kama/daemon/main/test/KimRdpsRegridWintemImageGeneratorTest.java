...
public class KimRdpsRegridWintemImageGeneratorTest {
	...
	public KimRdpsRegridWintemImageGeneratorTest() {
	...
	public BoundLonLat generateImage(NetcdfDataset ncFile, String fileName, String savePath) {
		...
		try {
			...
			for(int j=0 ; j<1 ; j++) {
				...
				Float[][] valuesUWind = GridCalcUtil.convertStorageToValues(varVWind.read(rangeList).getStorage(), rows, cols);
				Float[][] valuesVWind = GridCalcUtil.convertStorageToValues(varUWind.read(rangeList).getStorage(), rows, cols);
...
//						ig2.drawImage(wtailImageList[tailIndex][(int)((wd + 360) % 360) / 10], xPoint, yPoint, null);
	...
	public static void main(String[] args) {
		...
		KimRdpsRegridWintemImageGeneratorTest kimRdpsImageGeneratorTest = new KimRdpsRegridWintemImageGeneratorTest();
                  
		try {
						
			NetcdfDataset ncFile = NetcdfDataset.acquireDataset("F:/KAMA_AAMI/2025/항기청_수신/항기청_수신_20250602/r030_v040_ne36_pres_h000.2025060200.gb2", null);
			String fileName = "r030_v040_ne36_pres_h000.2025060200.gb2";
	        ...
	        kimRdpsImageGeneratorTest.generateImages(ncFile, fileName, savePath, paramMap1);
	        ...
	        kimRdpsImageGeneratorTest.generateImages(ncFile, fileName, savePath, paramMap2);
