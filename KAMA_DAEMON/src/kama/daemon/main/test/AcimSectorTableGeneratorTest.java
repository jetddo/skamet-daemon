...
public class AcimSectorTableGeneratorTest {
	...
	private List<NetcdfDataset> getAcimModelFileList() {
		...
		try {
			...
			Date issuedTm = sdf.parse("2025071212");  

			String modelFilePath = "F:/data/datastore/ACIM/" + sdf2.format(issuedTm); // 모델 파일 경로 설정
			...
			for (File file : modelDir.listFiles()) {
				if (file.isFile() && file.getName().startsWith("amo_gdum_acim_f")
			...
			Collections.sort(modelFileList, (f1, f2) -> {
                ...
                try {
                	...
                	//F:/data/datastore/ACIM/2025/06/19/00/amo_gdum_acim_f00_2025061900.nc
	...
	private void generateSectorTable() {
        ...
        for (int j = 0; j < fcstHourSize+1; j++) {  
			...
			if (j > 0) {
				...
				Variable var = ncFile.findVariable("CLTOP"); // CLTOP 변수 찾기
	...
	public static void main(String[] args) {
		...
		AcimSectorTableGeneratorTest acim = new AcimSectorTableGeneratorTest();
