 ...
 */
public class UpdateIcingKpAirepVerify {
	
	private SimpleDateFormat logDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
	
	private String[] icingKpModelDirPatterns = new String[]{"[0-9]{4}", "[0-9]{2}", "[0-9]{2}", "[0-9]{2}"};
	...
	private final String insertIcingKpAirepVerifyOracleQuery = 
			
			" INSERT INTO AAMI.ICING_KP_AIREP_VERIFY(TM, STN_CD, ACFT_TYPE, LAT, LNGT, FLIGHT_LEVEL, ICING, MODEL_ISSUED_DT, MODEL_FCST_DT, MODEL_ICING) VALUES " + 
	...
	private boolean initialize() {
		...
		try {
			...
			String coordinatesLatPath = this.config.getString("icing_kp.coordinates.lat.path");
			String coordinatesLonPath = this.config.getString("icing_kp.coordinates.lon.path");
			
			modelGridUtil = new ModelGridUtil(ModelGridUtil.Model.ICING_KP, null, coordinatesLatPath, coordinatesLonPath);
			...
			System.out.println("Error : UpdateIcingKpAirepVerify.initialize -> " + e);
	...
	private void process() {
		...
		if(!this.initialize()) {
			...
			System.out.println("Error : UpdateIcingKpAirepVerify.process -> initialize failed");
			return;
		}
		...
		storePath = storePath + File.separator + "ICING_KP";
		...
		System.out.println(this.logDateFormat.format(new Date(System.currentTimeMillis())) + " -> ::::: Insert IcingKp Verify Data :::::");
...
//				tm = new Date(tm.getTime() - (1000 * 60 * 60 * 9));
				...
				if(Math.abs(boundLonLat.getTop() - latitude) > latInterval || Math.abs(boundLonLat.getLeft() - longitude) > lonInterval) {
					System.out.println("Error : UpdateIcingKpAirepVerify.process -> position is too far");
					continue;
				}
				...
				List<Map<String, Object>> icingKpModelFileInfoList = getIcingKpModelFileInfoListbyTm(storePath, tm);
				
				for(int j=0 ; j<icingKpModelFileInfoList.size() ; j++) {
					
					Map<String, Object> icingKpModelFileInfo = icingKpModelFileInfoList.get(j);
					
					String icingKpFilePath = (String)icingKpModelFileInfo.get("icingKpFilePath");
					
					Date issuedDt = (Date)icingKpModelFileInfo.get("issuedDt");
					Date fcstDt = (Date)icingKpModelFileInfo.get("fcstDt");
					
					NetcdfDataset ncFile = NetcdfDataset.acquireDataset(icingKpFilePath, null);
					
					Variable var = ncFile.findVariable("RAP");
					
					float v = ((float[])var.read(rangeList).getStorage())[0];
					
					String query = MessageFormat.format(this.insertIcingKpAirepVerifyOracleQuery, new Object[]{
						sdf.format(tm),
						stnCd,
						acftType,
						lat,
				...
				System.out.println("Error : UpdateIcingKpAirepVerify.process -> " + e);
				e.printStackTrace();
				continue;
			}
		}
	...
	private List<Map<String, Object>> getIcingKpModelFileInfoListbyTm(String storePath, Date tm) {
		
		List<Map<String, Object>> icingKpModelFileInfoList = new ArrayList<Map<String, Object>>();
		...
		recursiveProc(icingKpModelFileInfoList, rootDir, tm, 0);
		
		return icingKpModelFileInfoList;
	}
	
	private void recursiveProc(List<Map<String, Object>> icingKpModelFileInfoList, final File baseDir, final Date tm, int depth) {
		
		File[] dirs = baseDir.listFiles();
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHH");
		
		String pattern = this.icingKpModelDirPatterns[depth];
		
		for(File dir : dirs) {
			
			if(dir.isDirectory() && dir.getName().matches(pattern)) {	
				
				if(depth == this.icingKpModelDirPatterns.length-1) {
					...
					try {
						...
						//(.+h)([0-9]{2})(.[0-9]{10}.nc)
						File[] icingKpModelFiles = dir.listFiles(new FilenameFilter() {

							@Override
							public boolean accept(File dir, String name) {
												
								if(name.matches("air_ice0_umlg_h[0-9]{2}.[0-9]{10}.nc")) {
									return true;
								}
						...
						Arrays.sort(icingKpModelFiles);
						
						for(int i=0 ; i<icingKpModelFiles.length ; i++) {
							
							File icingKpModelFile = icingKpModelFiles[i];
							
							Date fcstDt = new Date(issuedDt.getTime() + (1000 * 60 * 60) * 
									Integer.valueOf(icingKpModelFile.getName().replaceAll("(.+h)([0-9]{2})(.[0-9]{10}.nc)", "$2")));
								
							float interval = (tm.getTime() - fcstDt.getTime());
								
							if(interval >= 0 && interval < 3 * 60 * 60 * 1000) {
								
								Map<String, Object> icingKpModelFileInfo = new HashMap<String, Object>();
								
								icingKpModelFileInfo.put("icingKpFilePath", icingKpModelFile.getAbsolutePath());
								icingKpModelFileInfo.put("issuedDt", issuedDt);
								icingKpModelFileInfo.put("fcstDt", fcstDt);
								
								icingKpModelFileInfoList.add(icingKpModelFileInfo);
							}
						}						
				...
				} else if(depth < this.icingKpModelDirPatterns.length-1) {
					recursiveProc(icingKpModelFileInfoList, dir, tm, depth+1);
				}
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		new UpdateIcingKpAirepVerify().process();
	}
}